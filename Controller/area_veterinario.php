...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($acao == 'cancelar_agendamento') {
        ...
        $stmt = $pdo->prepare("SELECT observacoes FROM agendamentos WHERE id = ?");
        $stmt->execute([$agendamento_id]);
        $observacoesAtuais = $stmt->fetchColumn() ?: '';
        $novasObservacoes = trim($observacoesAtuais . "\nCancelado pelo veterinário: " . $motivo);

        $stmt = $pdo->prepare("UPDATE agendamentos SET status = 'cancelado', observacoes = ? WHERE id = ?");
        if ($stmt->execute([$novasObservacoes, $agendamento_id])) {
    ...
    if ($acao == 'concluir_agendamento') {
        ...
        $stmt->execute([$agendamento_id]);
        $observacoesAtuais = $stmt->fetchColumn() ?: '';
        $novasObservacoes = trim($observacoesAtuais . "\nConsulta realizada: " . $observacoes_consulta);

        $stmt = $pdo->prepare("UPDATE agendamentos SET status = 'concluido', observacoes = ? WHERE id = ?");
        if ($stmt->execute([$novasObservacoes, $agendamento_id])) {
...
$stmt = $pdo->prepare("
    ...
    JOIN pets p ON a.pet_id = p.id
    JOIN servicos s ON a.servico_id = s.id
    WHERE a.veterinario_id = ? AND a.data_agendamento > ? AND a.data_agendamento <= ?
    AND a.status IN ('pendente', 'confirmado')
    ORDER BY a.data_agendamento, a.hora_agendamento
");
$stmt->execute([$veterinario['id'], $hoje, $limite]);
...
$stmt = $pdo->prepare("
    ...
    FROM agendamentos 
    WHERE veterinario_id = ? AND data_agendamento >= ? AND data_agendamento <= ?
");
$stmt->execute([$veterinario['id'], $inicio_mes, $fim_mes]);
